...
# This helper loads the saved vocabulary metadata produced by data preparation.
def load_meta(config: GPTConfig) -> dict:
    with resolve_path(config.meta_file).open("rb") as handle:
...
def main() -> None:
    ...
    runtime_config = GPTConfig(**{**config.to_dict(), "vocab_size": meta["vocab_size"]})
    ...
    for iteration in range(runtime_config.max_iters):
        ...
        if iteration % runtime_config.eval_interval == 0 or iteration == runtime_config.max_iters - 1:
            ...
            # This block saves the checkpoint whenever validation loss improves (early stopping).
            improved = ""
